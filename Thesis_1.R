...
getwd()
setwd("/Users/Christophe/Desktop/Thesis")
# Financial Data without all potential control variables: FinancialData <- read.csv(file="Compustat_Financial_Data_Wharton.csv", header=TRUE, sep=",") # extracted from Wharton database
...
# -------
# CIK associated to the gvkey
getwd()
setwd("/Users/Christophe/Desktop/Thesis")
CIK_Draft <- as.data.frame(read.csv(file="CIK_GVKEY.csv", header=TRUE, sep=",")) #CIK associated to the gvkey downloaded
# from Compustat (Wharton)
CIK <- as.data.frame(CIK_Draft[,1])
CIK[,2] <- as.data.frame(CIK_Draft[,12])
CIK_Unique<-unique(CIK, incomparables = FALSE)
colnames(CIK_Unique)<-c("Gvkey", "CIK")
write.csv(CIK_Unique, file = "CIK_GVKEY_Unique.csv")
