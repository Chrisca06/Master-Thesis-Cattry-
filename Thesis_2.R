...
# ---------- Match CSTATSEC and Merging File 1 ---------- 

getwd()
setwd("/Users/Christophe/Desktop/Thesis")

CIKGVKEY <- read.csv(file="GVKEYSEC.csv", header=TRUE, sep=",")
CIKGVKEY$X.1 <- NULL
CIKGVKEY$X.2 <- NULL
CIKGVKEY$X.3 <- NULL
...
C <- cbind(Merging[1,],CIKGVKEY[1,])
C <- C[0,]

# gives the position: match(Merging[1,3], CIKGVKEY[,3])

for (i in 1:nrow(Merging)) {
  Pos <- match(Merging[i,3], CIKGVKEY[,3])
  C[i,] <- cbind(Merging[i,], CIKGVKEY[Pos,])
}

write.csv(C, file = "Second_Merging.csv")

# new file: 
Merging_Final <- read.csv(file="Second_Merging.csv", header=TRUE, sep=",")

# ---------- Clean Final Merging File ---------- 

setwd("/Users/Christophe/Desktop/Thesis")
Merging_Final <- read.csv(file="Second_Merging.csv", header=TRUE, sep=",")

Merging_Final$X<-NULL
Merging_Final$X.1<-NULL
Merging_Final$X.2<-NULL
Merging_Final$Index<-NULL
Merging_Final$Index.1<-NULL
Merging_Final$websiteSEC<-NULL
Merging_Final$formtype<-NULL
Merging_Final$datereport<-NULL
Merging_Final$datefiled<-NULL
Merging_Final$Index.2<-NULL
Merging_Final$quarter<-NULL

# Final Merging file (CSTATViolation with Financial Data and CIKSEC associated to each Gvkey)

write.csv(Merging_Final, file = "Final_Merging.csv")
