...
New_Violation[,1:2] <- NULL
New_Violation$consol <- NULL
New_Violation$popsrc <- NULL
New_Violation$datafmt <- NULL

write.csv(New_Violation, file="Violation_Data.csv")

# -------------------- Download Accept Time---------------------------
library(rvest)
library(xml2)
library(XML)

setwd("/Users/Christophe/Desktop/Thesis")
Viol <- read.csv(file="Violation_Data.csv", header=TRUE, sep=",")
# Index.Correct is the column with the url to the index website
# Creat new column in data that will host the timing
Viol$time <- NA

for (i in 1:nrow(Viol)){
  index <- Viol$Index.Correct[i]
  index <- read_html(toString(index))
  parsing <-  html_nodes(index,'.info')
  Time <- html_text(parsing[2])
  Viol$time[i] <- Time
  remove(index)
  remove(parsing)
  remove(Time)
...
# need to retrieve the last 8 digits for the time
install.packages("stringr")
library(stringr)
Viol$time.2 <- NA

for (i in 1:nrow(Viol)){
  Viol$time.2[i]<- str_sub(Viol$time[i],-8,-1)
}

write.csv(Viol,file="Violation_Data_2.csv")
